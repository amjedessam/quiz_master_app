...
//       avatar:
//           'https://ui-avatars.com/api/?name=${nameController.text.trim()}&background=6C63FF&color=fff',
//       createdAt: DateTime.now(),
//     );

//     await _storageService.saveUser(newUser);
//     await _storageService.saveToken('mock_token_${newUser.id}');
...
class RegisterController extends GetxController {
  ...
  Future<void> register() async {
    ...
    // إنشاء مستخدم بالبيانات الحقيقية
    final newUser = UserModel(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      name: nameController.text.trim(), // الاسم الحقيقي
      email: emailController.text.trim(), // البريد الحقيقي
      avatar:
