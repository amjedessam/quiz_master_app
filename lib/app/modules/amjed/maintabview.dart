...
// import 'package:quiz_master_app/app/modules/quiz_setup/quiz_setup_controller.dart';
...
import '../summaries/summaries_controller.dart';
...
class MainTabView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // تسجيل SummariesController إذا لم يكن مسجلاً مسبقًا
    if (!Get.isRegistered<SummariesController>()) {
      Get.put(SummariesController());
    }

    return DefaultTabController(
      length: 2, // Tab خارجي: الملخصات والشروحات / الاختبار
      child: Scaffold(
        appBar: AppBar(
          automaticallyImplyLeading: false, // لا نحتاج زر العودة
          // title: const Text('الملخصات والشروحات / الاختبار'),
          centerTitle: true,
          title: const TabBar(
            tabs: [
              Tab(text: 'الملخصات والشروحات'),
              Tab(text: 'الاختبار'),
            ],
          ),
        ),
        body: const TabBarView(
          children: [
            SummariesView(showAppBar: false), // التاب الأول: الملخصات والشروحات

            QuizWithInnerTab(), // التاب الثاني: الاختبار مع Tab داخلي
          ],
        ),
      ),
    );
...
/// التاب الداخلي الخاص بالاختبار
class QuizWithInnerTab extends StatelessWidget {
  const QuizWithInnerTab({super.key});

  @override
  Widget build(BuildContext context) {
    return DefaultTabController(
      length: 2, // Tab داخلي: الاختبار / سجل الاختبارات
      child: Scaffold(
        // appBar: AppBar(
        //   automaticallyImplyLeading: false, // لا نحتاج زر العودة
        //   bottom: const TabBar(
        ...
        body: TabBarView(
          children: [
            // Tab الاختبار: تسجيل QuizSetupController تلقائيًا
            Builder(
              builder: (context) {
                if (!Get.isRegistered<QuizSetupController>()) {
                  Get.put(QuizSetupController());
                }
                return const QuizSetupView();
              },
            ),

            // Tab السجل: تسجيل HistoryController تلقائيًا
            Builder(
              builder: (context) {
                if (!Get.isRegistered<HistoryController>()) {
                  Get.lazyPut(() => HistoryController());
